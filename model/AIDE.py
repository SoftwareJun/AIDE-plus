...
class AIDE_Model(nn.Module):
    ...
    def __init__(self, resnet_path, convnext_path, sr_path):
        ...
        #다음번에 할 때는 main finetunee 안건드리고 sr_path여기서
        #loading weights만 구현하면 됨
    ...
    def forward(self, x):
        ...
        tokens = x
        x_minmin, x_maxmax, x_minmin1, x_maxmax1 = self.dct(x)

        x_minmin = self.resize(x_minmin)
        x_maxmax = self.resize(x_maxmax)
        x_minmin1 = self.resize(x_minmin1)
        x_maxmax1 = self.resize(x_maxmax1)

        x_minmin = (x_minmin - imagenet_mean) / imagenet_std
        x_maxmax = (x_maxmax - imagenet_mean) / imagenet_std
        x_minmin1 = (x_minmin1 - imagenet_mean) / imagenet_std
        x_maxmax1 = (x_maxmax1 - imagenet_mean) / imagenet_std
...
def AIDE(resnet_path, convnext_path, sr_path):
    model = AIDE_Model(resnet_path, convnext_path, sr_path)
    return model
